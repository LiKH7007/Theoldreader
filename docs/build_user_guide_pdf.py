...
def main() -> None:
    ...
    story.append(p(
        ...
        "若配置 OpenAI 或 DeepSeek，则邮件按期刊分组输出；每篇文献会列出中文题名、英文题名、作者/一作/通讯作者、来源，并按材料/体系、新现象/机制、为什么重要、证据来源、建议五项输出，同时给出重要性标签。"
        "邮件会同时包含 HTML 正文和纯文本备用版本；普通邮箱会优先显示带加粗和层级的 HTML。"
