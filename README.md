...
----------------------------------------------------------------
这个仓库用于每天自动读取 The Old Reader 的订阅源最新更新，并把摘要整理成 HTML 邮件发送。邮件同时保留纯文本备用版本；普通邮箱会优先显示带层级和加粗的 HTML 正文。
...
OpenAI / DeepSeek 模式下，邮件会按期刊分组输出，而不是先按 `[必读]`、`[值得下载]` 大段分组：
...
邮件正文是 HTML 渲染，期刊标题、论文标题和字段名会真正加粗；每篇文章内部尽量紧凑，不额外插入大段空行。
...
OpenAI / DeepSeek 模式下，每篇进入正文的研究条目都必须按下面五项输出，并给出 `[高]` / `[中高]` / `[中]` / `[低]` 重要性标签：
